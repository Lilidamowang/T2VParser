...
class VITCLIPPretrained(nn.Module):
    def __init__(self, pretrained_model="weight/clip-vit-base-patch16", clip_weight=None, return_mean=True, 
                 patch_3d=False, return_all=False, **kwargs):
        super().__init__()
        print ("==== CLIP Vision ====")
        print (f"with {pretrained_model}")
        if clip_weight:
    ...
    # @TODO:
    def forward(self, x, return_all=False, **kwargs): # x.shape = [16, 12 , 3, 224, 224]
        if x.ndim == 5: 
            ...
            if x.shape[1]==3:
                ...
                B, T, D, H, W = x.shape   
            x = x.reshape((-1,) + x.shape[2:]) # x.shape = [192, 3, 224, 224]
        else:
            B, _, _, _ = x.shape
            T = 1
        self.T = T
...
@MODELS.register_module()
class VITCLIPPretrained_STAN(VITCLIPPretrained): # TODO:StAN model vision part
    def __init__(self, depth=4, cls_residue=False, time_module="selfattn",
        pretrained_model=None, clip_weight=None, all_patch=False,
        gradient_checkpointing=False, **kwargs): 
        super().__init__(clip_weight=clip_weight, **kwargs)
        # pretrained_model = "weight/clip-vit-base-patch32"
        if clip_weight:
            configuration = CLIPConfig().from_pretrained(clip_weight)
        else:
            configuration = CLIPConfig().from_pretrained(pretrained_model)
