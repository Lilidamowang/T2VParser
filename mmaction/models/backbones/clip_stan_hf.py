...
class VITCLIPPretrained(nn.Module):
    def __init__(self, pretrained_model, clip_weight=None, return_mean=False, 
...
class VITCLIPPretrained_STAN(VITCLIPPretrained): 
    def __init__(self, depth=4, cls_residue=False, time_module="selfattn",
        ...
        super().__init__(clip_weight=clip_weight, pretrained_model=pretrained_model, **kwargs)
    ...
    def forward_patch(self, x):
        ...
        cls_token = x[:, 0] + x2[:, 0].repeat(1, self.T).view(x2.size(0) * self.T, -1) # [96, 768]
        cls_token = self.post_layernorm(cls_token)
