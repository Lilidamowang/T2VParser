...
# T2VParser MSRVTT dataset example
...
class MsrvttDataset(BaseActionDataset):
    ...
    def load_data_list(self) -> List[Dict]:
        ...
        if self.ann_file.endswith("train_9k.json"):
            all_train_id = []
            with open("../msrvtt_data/MSRVTT_train.9k.csv", 'r') as f:
                reader = csv.reader(f)
                for row in reader:
                    all_train_id.append(row[0])
            with open("../msrvtt_data/MSRVTT_data.json", 'r') as f:
                res = json.load(f)
                ...
                for item in sentences:
                    ...
                    if vid in all_train_id:
                        vpath = f"../msrvtt_data/msrvttvideo/{vid}.mp4"
                        data_item = dict(filename=vpath, text=cap)
                        video_text_pairs=[]
                        video_text_pairs.append(data_item)
                        data_list.extend(video_text_pairs)
            # extra captions
            with open('../msrvtt_data/msrvtt_doc.csv', 'r') as file:  # extra captions
                reader = csv.reader(file)
                for row in reader:
                    if row[0] in all_train_id:
                        vpath = f"../root/autodl-tmp/data/msrvtt_data/msrvttvideo/{row[0]}.mp4"
            ...
            with open("../msrvtt_data/MSRVTT_JSFUSION_test.csv", 'r') as f:
                reader = csv.reader(f)
                all_row = []
                for row in reader:
                    all_row.append(row)
            ...
            for item in all_row:
                vid = item[2]
                cap = item[3]
                vpath = f"../root/autodl-tmp/data/msrvtt_data/msrvttvideo/{vid}.mp4"
                data_item = dict(filename=vpath, text=cap)
                video_text_pairs=[]
                video_text_pairs.append(data_item)
                data_list.extend(video_text_pairs)
